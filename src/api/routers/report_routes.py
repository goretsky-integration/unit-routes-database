...
from services.report_routes import (
    group_report_routes_by_report_type_and_chat_id
)
...
def get_report_routes(
        ...
        report_routes: ReportRouteRepository = Depends(
            dependencies.get_report_routes_repository),
):
    report_routes_from_db = report_routes.get_all(skip=skip, limit=limit,
                                                  chat_id=chat_id,
...
def create_report_route(
        ...
        telegram_chats: TelegramChatRepository = Depends(
            dependencies.get_telegram_chats_repository),
        report_types: ReportTypeRepository = Depends(
            dependencies.get_report_types_repository),
        report_routes: ReportRouteRepository = Depends(
            dependencies.get_report_routes_repository),
):
    telegram_chat, _ = telegram_chats.get_or_create(
        chat_id=report_route.chat_id)
...
@router.get(path='/telegram-chats/')
def get_telegram_chats_subscribed_to_unit_by_report_type(
        report_routes: ReportRouteRepository = Depends(
            dependencies.get_report_routes_repository,
        ),
        unit_id: int = Query(),
    ...
    return report_routes.get_telegram_chat_ids(
        unit_id=unit_id,
        report_type_name=report_type,
    )
